...
function staticType(file){const e=path.extname(file).toLowerCase();return e==='.html'?'text/html; charset=utf-8':e==='.js'?'application/javascript; charset=utf-8':e==='.css'?'text/css; charset=utf-8':e==='.png'?'image/png':'application/octet-stream';}
function safeStaticFile(publicDir,urlPath){const root=path.resolve(publicDir);const rel=path.normalize(urlPath==='/'?'index.html':urlPath.replace(/^[/\\]+/,''));const file=path.resolve(root,rel);const relative=path.relative(root,file);if(relative.startsWith('..')||path.isAbsolute(relative))return null;return file;}
function createHttpServer({director,publicDir,mapConfig={}}){return http.createServer((req,res)=>{try{const u=new URL(req.url,'http://localhost');if(u.pathname==='/api/snapshot')return sendJson(res,200,director.snapshot());if(u.pathname==='/api/map-snapshot')return sendJson(res,200,director.compactSnapshot());if(u.pathname.startsWith('/api/npc/')){const id=decodeURIComponent(u.pathname.slice('/api/npc/'.length));const d=director.npcDetails(id);return d?sendJson(res,200,d):sendJson(res,404,{error:'npc not found'});}if(u.pathname==='/api/health')return sendJson(res,200,director.healthSnapshot());if(u.pathname==='/api/map-config')return sendJson(res,200,mapConfig);if(u.pathname==='/api/teleport'){try{const x=finiteNumber(u.searchParams.get('x'),'x'),y=finiteNumber(u.searchParams.get('y'),'y');const rawZ=u.searchParams.get('z');if(rawZ==null||rawZ==='')throw new TypeError('z is required because SCUM #Teleport requires X Y Z');const z=finiteNumber(rawZ,'z');const player=u.searchParams.get('player')||'';return sendJson(res,200,{command:buildTeleportCommand({x,y,z},player)});}catch(err){return sendJson(res,400,{error:err.message});}}if(!publicDir)return sendJson(res,404,{error:'not found'});const file=safeStaticFile(publicDir,u.pathname);if(!file||!fs.existsSync(file)||fs.statSync(file).isDirectory())return sendJson(res,404,{error:'not found'});res.writeHead(200,{'content-type':staticType(file)});fs.createReadStream(file).pipe(res);}catch(err){sendJson(res,500,{error:err.message});}});}
class FileIpcReader{
 constructor(file,offsetFile=null){this.file=file;this.offsetFile=offsetFile;this.offset=0;this.fileId=null;this._loadOffset();if(!this.fileId&&fs.existsSync(this.file)){try{this.fileId=this._fileId(fs.statSync(this.file));}catch{}}}
 _fileId(st){if(!st)return null;return `${st.dev??''}:${st.ino??''}:${st.birthtimeMs??''}`;}
 _loadOffset(){if(!this.offsetFile||!fs.existsSync(this.offsetFile))return;try{const d=JSON.parse(fs.readFileSync(this.offsetFile,'utf8'));if(d.file===path.resolve(this.file)&&Number.isSafeInteger(d.offset)&&d.offset>=0){this.offset=d.offset;this.fileId=typeof d.fileId==='string'&&d.fileId?d.fileId:null;}}catch{} }
 _saveOffset(){if(!this.offsetFile)return;fs.mkdirSync(path.dirname(this.offsetFile),{recursive:true});const tmp=this.offsetFile+'.tmp';fs.writeFileSync(tmp,JSON.stringify({version:2,file:path.resolve(this.file),fileId:this.fileId,offset:this.offset,updatedAt:Date.now()}));fs.renameSync(tmp,this.offsetFile);}
 _readComplete(file,start,onEvent){if(!fs.existsSync(file))return start;const st=fs.statSync(file);if(st.size<=start)return start;const fd=fs.openSync(file,'r');try{const len=st.size-start;const buf=Buffer.alloc(len);fs.readSync(fd,buf,0,len,start);let lastNl=-1;for(let i=buf.length-1;i>=0;i--){if(buf[i]===10){lastNl=i;break;}}if(lastNl<0)return start;const complete=buf.subarray(0,lastNl+1).toString('utf8');for(const line of complete.split(/\r?\n/)){if(!line.trim())continue;const e=parseEventLine(line);if(e)onEvent(e);}return start+lastNl+1;}finally{fs.closeSync(fd);}}
 poll(onEvent){if(!fs.existsSync(this.file))return;let st=fs.statSync(this.file);const currentId=this._fileId(st);const replaced=Boolean(this.fileId&&currentId&&this.fileId!==currentId);if(replaced||st.size<this.offset){const rotated=this.file+'.1';if(fs.existsSync(rotated))this._readComplete(rotated,this.offset,onEvent);this.offset=0;this.fileId=currentId;this._saveOffset();st=fs.statSync(this.file);}else if(!this.fileId){this.fileId=currentId;this._saveOffset();}if(st.size===this.offset)return;this.offset=this._readComplete(this.file,this.offset,onEvent);this.fileId=currentId;this._saveOffset();}
}
class StateSnapshotReader{
 constructor(file){this.file=file;this.lastMtimeMs=0;}
