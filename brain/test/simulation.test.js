...
test('nearby observed NPCs become a 1-5 member group with leader and class',()=>{
 const d=new WorldDirector({seed:'sim'}); seen(d,'a','BP_Guard_Lvl_4',0,0); seen(d,'b','BP_Guard_Lvl_4',200,100); seen(d,'c','BP_Guard_Lvl_3',300,50);
...
test('radiation body maps to radiation specialist and radiation group class',()=>{
 const d=new WorldDirector({seed:'sim'}); seen(d,'r1','BP_Guard_Lvl_5_Radiation',0,0); seen(d,'r2','BP_Guard_Lvl_4_Radiation',100,0);
...
test('gunshot world event raises stress by distance without hardcoding combat action',()=>{
 const d=new WorldDirector({seed:'sim'}); seen(d,'a','BP_Drifter_Lvl_1',0,0); seen(d,'b','BP_Drifter_Lvl_1',50000,0); const beforeA=d.world.npcs.a.stress,beforeB=d.world.npcs.b.stress; d.ingest({type:'GUNSHOT',x:100,y:0,z:0,intensity:1,at:Date.now()}); assert.ok(d.world.npcs.a.stress>beforeA); assert.ok(d.world.npcs.a.stress>d.world.npcs.b.stress-beforeB);
});
