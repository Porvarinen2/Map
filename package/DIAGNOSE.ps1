...
if ($out) {
  ...
  foreach ($f in @("boot.log", "director.log", "events.tsv", "movement_debug.tsv", "last_engine_calls.txt", "live_state.json", "npc_api.txt", "npc_loadout.txt", "player_outfit.txt", "item_classes.txt")) {
    $p = Join-Path $out $f
    if (Test-Path $p) {
